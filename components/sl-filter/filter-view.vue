  ...
  <view>
    <view style="padding: 0px 0px">
      <view
        ...
        :key="index"
      >
        <view v-if="menuIndex == index">
          <view v-if="item.mySl">
            <select-list
              @getSelectItem="getSelectItem($event, item.key)"
            ></select-list>
          </view>
          <view v-else-if="item.myCity">
            <select-city
              v-model="value"
              :hot-citys="hotCitys"
              @on-select="getCity($event, item.key)"
            ></select-city>
          </view>
          <view v-else-if="item.isSort">
            <view class="filter-content-list">
              <view
                ...
                "
                :style="{
                  color: detailItem.isSelected ? themeColor : '#666666',
                }"
          ...
          <view v-else>
            <view
              ...
              v-if="item.detailTitle && item.detailTitle.length"
            >
...
export default {
  ...
  methods: {
        ...
        if (
          ...
          if (item.isMutiple) {
            ...
            for (let j = 0; j < item.defaultSelectedIndex.length; j++) {
              ...
              obj[item.key].push(
                item.detailList[item.defaultSelectedIndex[j]].value
              )
            }
          } else {
      ...
      for (let i = 0; i < this.menuList.length; i++) {
        ...
        if (this.menuList[i].defaultSelectedIndex) {
          if (Array.isArray(this.menuList[i].defaultSelectedIndex)) {
            ...
            for (
              let j = 0;
              j < this.menuList[i].defaultSelectedIndex.length;
              j++
            ) {
              if (
                this.selectDetailList[this.menuList[i].defaultSelectedIndex[j]]
                  .isSelected == false
              ) {
                this.itemTap(
          ...
          for (let j = 0; j < unDefaultSelectedIndexArr.length; j++) {
            if (
              this.selectDetailList[unDefaultSelectedIndexArr[j]].isSelected ==
              true
            ) {
              this.itemTap(
      ...
      if (isMutiple == true) {
        ...
        if (index == 0) {
          ...
          if (list[index].isSelected) {
            ...
            if (this.independence) {
              var idx = this.independenceObj[this.selectedKey].indexOf(
                list[index].value
              )
    ...
    //科室选择
    getSelectItem(val, key) {
      // index, list, key
      console.log(val)
      this.selectedObj[key] = val.id
      this.result = this.selectedObj
      this.selectedTitleObj[key] = val.title
    ...
    getCity(val, key) {
      console.log(val, key)
      this.selectedObj[key] = val
      this.result = this.selectedObj
      this.selectedTitleObj[key] = val
